...
import { getAllBookings, upcomingBookings } from "../../../redux/slice/bookingSlice";
 ...
 const navigate = useNavigate()
  ...
  const upcomingBooking = [
    ...
    },
    {
      Header: "Name",
      Cell: ({ row }) => (
        <span className="text-capitalize fw-bold cursor" onClick={() => handleUserInfo(row?.userId?row?.userId?._id : "-")} >
          {row?.userId? row?.userId?.fname + " " + row?.userId?.lname : 'Salon User'}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      Cell: ({ row }) => (
        <div>
          {row?.serviceId && row.serviceId.map((service, index) => (
    ...
    },
    {
      Header: "Commission",
      Cell: ({ row }) => (
        <span className="text-capitalize">{row?.status == "cancel" ? "-" : row?.commission + " " + setting?.currencySymbol}</span>
      ),
    },
    ...
    },
    {
      Header: "Last Slot",
      Cell: ({ row }) => (
        <span className="text-capitalize">{row.time[row.time.length - 1]}</span>
      ),
    },
  ];
  const handleInfo = (id) => {
    navigate("/salonPanel/expert/getExpertProfile" ,{
      state : {
        id
      }
    })
  ...
  const handleUserInfo = (id) => {
    navigate("/salonPanel/user/userProfile" ,{
      state : {
        id
      }
    })
