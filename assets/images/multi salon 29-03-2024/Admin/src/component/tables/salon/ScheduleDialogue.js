...
import { updateSalonTime } from "../../../redux/slice/salonSlice";
...
const ScheduleDialogue = () => {
  ...
  if (dialogueData) {
    var timeInterval = dialogueData?.data?.breakTime;
    var [startTime, endTime] = timeInterval?.split(" TO ");
  }

  useEffect(() => {
    if (dialogueData) {
      setOpenTime(moment(dialogueData?.data?.openTime, "hh:mm A").format("hh:mm A"));
      setCloseTime(
  ...
  const handleSubmit = async (e) => {
    ...
    if (!openTime || !closeTime) {
      ...
      if (dialogueData) {
        ...
        const payload = { data: addTax, id: dialogueData?.salonId,day : dialogueData?.data?.day };
